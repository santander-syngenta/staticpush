...
def data_lake_sync():
	...
	###Sort By Year, Important when dropping duplicates
	output = output.sort_values(by='overview_setup_trialstartyear')
	##Drop duplicate Protocol ID's
	output = output.drop_duplicates(subset=['protocol_id'], keep='first')
	output.index = output['protocol_id']
	output = output.drop(columns=['protocol_id']).to_dict(orient='index')
	print('Data pulled from Data Lake successfully. Now Comparing to the protocol site data')
	for y in output.keys():
		try:
			protocol.objects.get(protocol_id = y)
			pass
		except:
...
def get_assessments(protocol_id):
	...
	df['collection_basis'] = df['collection_basis'].astype(float)
	df['reporting_basis'] = df['reporting_basis'].astype(float)
	...
	df['no_subsamples'] = df['no_subsamples'].astype(int)
...
def get_instructions(protocol_id):
	engine = pd_connect()
	p = protocol_id.upper()
	df = pd.read_sql("SELECT instructions_instructions as instructions, instructions_confidential as design_codes, instructions_studyruleset_code as study_rules FROM mio.public.mio212_trialprotocol_instructions WHERE protocol_id = \'" +p+'\';', engine)
	if df.empty == True:
		return [] 
	elif df.empty == False:
...
def clean_instructions(protocol_id):
	...
	if d == []:
		...
		return lines, output, out2, conf_out
...
def condense_pdoverview(protocol_id):
	overview_df, status_df = pd_overview(protocol_id)
...
def print_doc(protocol_id, shortname, font, font_size, confidential, color):
	#Pull Data From Data Lake & Pass Through 'Cleaning'
	df, totals = clean_trtdf(protocol_id)
	overview, status_df = condense_pdoverview(protocol_id)
	assessments = get_assessments(protocol_id)
	instructions = clean_instructions(protocol_id)

	d = Document()
	if overview != [] and status_df.empty == False:
		add_cover(d, protocol_id, overview, confidential, font, font_size, status_df)
	if instructions != []:
		instructions_cleaned = instructions[2]
		if confidential == True and instructions[3] != []:
			confidential_inst = instructions[3]
		else:
			confidential_inst = None 
		add_instructions2(d, instructions_cleaned, protocol_id,font_size,font, confidential_inst)
...
def add_instructions2(document, instructions, protocol_id,font_size, font, confidential_inst):
	d = instructions
	for header in d.keys():
		content = document.add_paragraph()
		try:
